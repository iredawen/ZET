...
def Position_Path(id_list, index,time_0,First, End,Path_to_Fianl_BaseStation,
    ...
    #print("参考的result_copy", result_copy)
...
def test(m = 150,
    ...
    for time_i in range(len(time_config)):  #遍历每一个时间点 3
        ...
        for station_j in range (len(station_config)): # 6 
            ...
            total = 0
            result_copy=None
            for j in range(4):   #决定是否路径复用 4.1 只用第一条  2.2 用1 2 条 
                if j <= 1: # j = 0 1 
                    for ij in range(1): #1 走两次 11
                        ...
                        result, final = Position_Path(id_list=path_list,index=jj,time_0= float(format(drone_ab.time_begin+j*(drone_ab.time_begin-time),'.4f')),
                    ...
                    drone_ab = path_list[1]
                    First_Drone =  (drone_ab.begin_m - 12, drone_ab.begin_n)
                    Final_Drone = (drone_ab.end_m - 12, drone_ab.end_n)
                    #final_time = drone_ab.time_begin+drone_ab.time_drone+drone_ab.time_end
                    Path_to_Fianl_BaseStation = [] #路径

                    result, final = Position_Path(id_list=path_list,index=jj,time_0= float(format(drone_ab.time_begin,'.4f')),
                                                                    First=First_Drone,End=Final_Drone, Path_to_Fianl_BaseStation=Path_to_Fianl_BaseStation,
                                                                    Station_x = Station_x, Station_y = Station_y,result_copy=result_copy)
                    ...
                    result, final = Position_Path(id_list=path_list,index=jj,time_0= float(format(drone_ab.time_begin+3*(drone_ab.time_begin-time),'.4f')),
                                                                    First=First_Drone,End=Final_Drone, Path_to_Fianl_BaseStation=Path_to_Fianl_BaseStation,
                                                                    Station_x = Station_x, Station_y = Station_y,result_copy=result_copy)
