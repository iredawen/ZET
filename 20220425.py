...
def Position_Path(id_list, index,time_0,final_time,First, End,Path_to_Fianl_BaseStation,
                                        d_intraOrbit = 90, d_interOrbit = 80, Station_x = True, Station_y = True,result_copy=None):
    FinalDrone = End
    FirstDrone = First
    id_list = id_list #路径的坐标
    index = index #路径的序号
    ...
    if index == 0: 
        ...
        if index <= 1 and (id_list[index].begin_m==id_list[0].begin_m or id_list[index].begin_n==id_list[0].begin_n) : #第2 条
            result_copy =result_copy
            result_copy.append((final_time,None,None))
            ...
            for j in range(a): #先行后列
                ...
                for re in range(len(result_copy)-1):
                    if m==result_copy[re][1] and n == result_copy[re][2] and t_last < result_copy[re][0]:
...
def test(m = 150,
    ...
    for time_i in range(len(time_config)):  #遍历每一个时间点 3
        ...
        for station_j in range (len(station_config)): # 6 
            ...
            print("timeid_list",timeid_list)
            print("drone_ab_list",drone_ab_list)
            ...
            for i in range(len(path_list)):
                print((path_list[i].begin_m,path_list[i].begin_n,path_list[i].end_m,path_list[i].end_n))
            ...
            if len(path_list) > 2: #11 2 3

                for ii in range(2):
                    drone_ab = path_list[0]
                    First_Drone =  (drone_ab.begin_m - 12, drone_ab.begin_n)
                    Final_Drone = (drone_ab.end_m - 12, drone_ab.end_n)
                    final_time = drone_ab.time_begin+drone_ab.time_drone+drone_ab.time_end
                    Path_to_Fianl_BaseStation = [] #路径
                    begin = Total_Decay(t=float(format(time,'.4f')), BaseStation_B=station_config[station_j][1],
                                                                    Decay=float(format(final_time-time+ii*(drone_ab.time_begin-time),'.4f')),
                                                                    BaseStation_A=station_config[station_j][0],j=2)
                    for i in range(len(begin)-1):
                        print(begin[i],end=",")
                    print(begin[i+1])

                    result = Position_Path(id_list=path_list,index=0,
                                                                    time_0= float(format(drone_ab.time_begin+ii*(drone_ab.time_begin-time),'.4f')),
                                                                    final_time=float(format(final_time-time+ii*(drone_ab.time_begin-time),'.4f')), 
                    ...
                    File(str(result[i+1]))
                    File('\n')
                result_copy = result.copy() #第一条路径的最后时刻点

                #路径 单独的                            
                for k in range(2):
                    drone_ab = path_list[k+1]
                    First_Drone =  (drone_ab.begin_m - 12, drone_ab.begin_n)
                    Final_Drone = (drone_ab.end_m - 12, drone_ab.end_n)
                    final_time = drone_ab.time_begin+drone_ab.time_drone+drone_ab.time_end
                    Path_to_Fianl_BaseStation = [] 
                    begin = Total_Decay(t=float(format(time,'.4f')), BaseStation_B=station_config[station_j][1],
                                                                    Decay=float(format(final_time-time,'.4f')),
                                                                    BaseStation_A=station_config[station_j][0],j=2+k)
                    for i in range(len(begin)-1):
                        print(begin[i],end=",")
                    print(begin[i+1])

                    result = Position_Path(id_list=path_list,index=k+1,time_0= float(format(drone_ab.time_begin,'.4f')),
                                                                    final_time=float(format(final_time,'.4f')), 
                                                                    First=First_Drone,End=Final_Drone, Path_to_Fianl_BaseStation=Path_to_Fianl_BaseStation,
                                                                    Station_x = Station_x, Station_y = Station_y,
                                                                    result_copy = result_copy)
                    ...
                    print(result[r+1])
                    #result_copy_id = result_copy_id+1
                ...
                for j in range(2):
                    for ij in range(2):
                        ...
                        drone_ab = path_list[ij]
                        First_Drone =  (drone_ab.begin_m - 12, drone_ab.begin_n)
                        Final_Drone = (drone_ab.end_m - 12, drone_ab.end_n)
                        final_time = drone_ab.time_begin+drone_ab.time_drone+drone_ab.time_end
                        Path_to_Fianl_BaseStation = [] #路径
                        begin = Total_Decay(t=float(format(time,'.4f')), BaseStation_B=station_config[station_j][1],
                                                                        Decay=float(format(final_time-time+j*(drone_ab.time_begin-time),'.4f')),
                                                                        BaseStation_A=station_config[station_j][0],j=jj)
                        for i in range(len(begin)-1):
                            print(begin[i],end=",")
                        print(begin[i+1])

                        result = Position_Path(id_list=path_list,index=ij,time_0= float(format(drone_ab.time_begin+j*(drone_ab.time_begin-time),'.4f')),
                                                                        final_time=float(format(final_time+j*(drone_ab.time_begin-time),'.4f')), 
                                                                        First=First_Drone,End=Final_Drone, Path_to_Fianl_BaseStation=Path_to_Fianl_BaseStation,
